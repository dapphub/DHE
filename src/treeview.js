...
export const MakeTabChildren = (tabs, typef) => {
  return tabs.map(
    (t, i) =>
      ({ index: i, state: t, selected: i === 0, type: typef && typef(t) })
  );
};
...
export const TabNav = TabNavView => ({ DOM, onion }) => {
  ...
  const view$ = xs
    .combine(onion.state$, tabView.DOM)
    .map(
      ([ p, view ]) =>
        div(
          ".navBtn",
          { class: { selected: p.selected }, attrs: { ref: p.signature } },
          [ view ]
        )
    );

  return { DOM: view$, select$ };
};

export const Stage = (C, sinkNames) => sources => {
  const selected$ = sources.onion.state$.map(s => s.selected);

  const ctype$ = xs
    .combine(sources.onion.state$, selected$)
    .filter(([ state, s ]) => s)
    .map(([ s ]) => s)
    .compose(componentSwitch(state => state.type, C, sources));

  const filterSelected = attr => in$ => in$
  ...
  const filterInstant = attr => in$ => xs
    .combine(selected$, in$.map(c => c.c[attr] || xs.of()).flatten())
    .filter(([ s ]) => s)
    .map(([ _, v ]) => v);

  const sinkObjects = sinkNames.map(
    sink => ctype$.compose(filterSelected(sink))
  );
...
export const Tabs = opt => sources => {
  const nav$ = sources.onion.state$.compose(
    member(TabNav(opt.TabNavView), {
      DOM: sources.DOM.isolateSource(sources.DOM, "tab"),
      onion: sources.onion
    })
  );

  const stage$ = sources.onion.state$.compose(
    member(Stage(opt.C, opt.sinkNames), sources)
  );
  ...
  const tabNavView$ = nav$
    .compose(pick((sinks, i) => sources.DOM.isolateSink(sinks.DOM)))
    .compose(mix(xs.combine));

  const sinkObjects = opt.sinkNames.map(
    sink => stage$.compose(pick(sinks => sinks[sink])).compose(mix(xs.merge))
  );

  var sinks = _.zipObject(opt.sinkNames, sinkObjects);

  const vdom$ = xs
    .combine(tabNavView$, tabStageView$)
    .map(
      ([ tabs, view ]) =>
        div(opt.classname, [
          div(".selectView", tabs),
  ...
  const selectReducer$ = nav$
    ...
    .compose(mix(xs.merge))
    .map(
      e => function selectReducer(parent) {
        ...
        tabs_[oldIndex] = _.assign({}, tabs_[oldIndex], { selected: false });
        tabs_[newIndex] = _.assign({}, tabs_[newIndex], { selected: true });
        return tabs_;
      }
    );
